...
measure-lemma0 : ∀ {a}{A : Set a}{V : Set a}⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄
              → (x : V)
              → (y : V)
              → (x ∙ ε ∙ y) ≡ x ∙ y
measure-lemma0 x y =
  ...
  ≡⟨ ∙-assoc x ε y ⟩
    (x ∙ ε) ∙ y
  ≡⟨ cong (λ z → z ∙ y) (ε-right x) ⟩
    x ∙ y
  ∎

measure-lemma1 : ∀ {a}{A : Set a}{V : Set a}⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄
              → (b : A)
              → ((mo Monoid.∙ Monoid.ε mo) (measure-digit (One b)) ≡ Measured.∥ m ∥ b)
measure-lemma1 ⦃ mo ⦄ ⦃ m ⦄ b =
  begin
    (mo Monoid.∙ Monoid.ε mo) (measure-digit (One b))
  ≡⟨ ε-left (measure-digit (One b)) ⟩
    ∥ b ∥
  ∎

assoc-lemma1 : ∀ {a}{A : Set a}{V : Set a}⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄
              → (a : A)
              → (e : A) →
              (mo Monoid.∙ Measured.∥ m ∥ a) (Measured.∥ m ∥ e) ≡ (mo Monoid.∙ Measured.∥ m ∥ a) ((mo Monoid.∙ Monoid.ε mo) (Measured.∥ m ∥ e))
...
assoc-lemma2 : ∀ {a}{A : Set a}{V : Set a}⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄ → (a : A) → (b : A) → (c : A) → (d : A) → (e : A) → (s : V) → (f : V)
        ...
        ((mo Monoid.∙
         ...
         s)
        f)
assoc-lemma2 a b c d e s f =
  begin
    ∥ a ∥ ∙ ((∥ b ∥ ∙ ∥ c ∥ ∙ ∥ d ∥ ∙ ∥ e ∥) ∙ (s ∙ f))
  ≡⟨ cong (λ x → ∥ a ∥ ∙ x) (sym (∙-assoc (∥ b ∥) (∥ c ∥ ∙ ∥ d ∥ ∙ ∥ e ∥) (s ∙ f))) ⟩
    ∥ a ∥ ∙ (∥ b ∥ ∙ (( ∥ c ∥ ∙ (∥ d ∥ ∙ ∥ e ∥)) ∙ (s ∙ f)))
  ≡⟨ ∙-assoc (∥ a ∥) (∥ b ∥) (( ∥ c ∥ ∙ (∥ d ∥ ∙ ∥ e ∥)) ∙ (s ∙ f)) ⟩
    (∥ a ∥ ∙ ∥ b ∥) ∙ (∥ c ∥ ∙ (∥ d ∥ ∙ ∥ e ∥)) ∙ (s ∙ f)
  ≡⟨ cong (λ x → (∥ a ∥ ∙ ∥ b ∥) ∙ x)
...
snoc-assoc-lemma4 m n p q r s t =
  begin
    ((m ∙ n ∙ p ∙ q ∙ (r ∙ s))) ∙ t
...
measure-dig-lemma0 : ∀ {a} {A : Set a} {V : Set a} ⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄
                → (d : Digit A)
...
measure-digit-lemma1 {s = s} pr ft sf =
  begin
    ((measure-digit pr) ∙ (s ∙ measure-digit sf))
...
splitDigit-size-lemma0 p i (One x) .nothing .x .nothing refl =
  begin
    ε ∙ ∥ x ∥ ∙ ε
  ≡⟨ ε-left (∥ x ∥ ∙ ε) ⟩
    ∥ x ∥ ∙ ε
...
assoc-lemma8 : ∀ {l}{V : Set l} ⦃ mo : Monoid V ⦄
            → (a : V) → (b : V) → (c : V) → (d : V) → (e : V) →
            (a ∙ b ∙ c) ∙ d ∙ e ≡ a ∙ b ∙ (c ∙ d ∙ e)
assoc-lemma8 {V = V} ⦃ mo ⦄ = assoc-lemma8'
  where
    open MonoidSolver V mo
    assoc-lemma8' = assoc-lemma-8ms
...
split-Tree2-measure-lemma p i pr ft sf l x r eq =
  begin
    measure-digit pr ∙ measure-tree ft ∙ measure-digit sf
  ≡⟨ cong (λ a → measure-digit pr ∙ measure-tree ft ∙ a) (sym (splitDigit-size-lemma0 p i sf l x r eq)) ⟩
    measure-digit pr ∙ measure-tree ft ∙ measure-maybe-digit l ∙ ∥ x ∥ ∙ measure-maybe-digit r
  ≡⟨ sym (assoc-lemma8 (measure-digit pr) (measure-tree ft) (measure-maybe-digit l) (∥ x ∥) (measure-maybe-digit r)) ⟩
    (measure-digit pr ∙ measure-tree ft ∙ measure-maybe-digit l) ∙ ∥ x ∥ ∙ measure-maybe-digit r
  ∎
...
mutual
  ...
  measure-node-digit-lemma : ∀ {a} {A : Set a} {V : Set a}
                            ⦃ mo : Monoid V ⦄
                            ⦃ m : Measured A V ⦄
                            → (n : Node A V)
                            → (measure-node n ≡ measure-digit (toDigit n))
  measure-node-digit-lemma (Node2 v x y p) = p
  measure-node-digit-lemma (Node3 v x y z p) = p


  assoc-lemma9 : ∀ {l} {V : Set l} ⦃ mo : Monoid V ⦄
      → (a : V) → (b : V) → (c : V) → (d : V) → (e : V) → (f : V) → (g : V)
      → (a ∙ b ∙ c) ∙ d ∙ (e ∙ f ∙ g) ≡ a ∙ (b ∙ (c ∙ d ∙ e) ∙ f) ∙ g
  assoc-lemma9 {V = V} ⦃ mo ⦄ = assoc-lemma9'
    where
      open MonoidSolver V mo
      assoc-lemma9' = assoc-lemma-9ms
  ...
  split-Tree3-measure-lemma p i pr sf ml mx mr l x r prf2 =
      begin
        (σpr ∙ (μ₁ ∙ (measure-node mx) ∙ μ₂) ∙ σsf)
      ≡⟨ cong (λ a → σpr ∙ (μ₁ ∙ a ∙ μ₂) ∙ σsf) (measure-node-digit-lemma mx) ⟩
        (σpr ∙ (μ₁ ∙ (measure-digit (toDigit mx)) ∙ μ₂) ∙ σsf)
      ≡⟨ cong (λ a → σpr ∙ (μ₁ ∙ a ∙ μ₂) ∙ σsf) (sym (splitDigit-size-lemma0 p (i ∙ σpr ∙ μ₁) (toDigit mx) l x r prf2)) ⟩
        σpr ∙ (μ₁ ∙ (measure-maybe-digit l ∙ ∥ x ∥ ∙ measure-maybe-digit r) ∙ μ₂) ∙ σsf
      ≡⟨ sym (assoc-lemma9 σpr μ₁ (measure-maybe-digit l) (∥ x ∥) (measure-maybe-digit r) μ₂ σsf) ⟩
        (σpr ∙ μ₁ ∙ (measure-maybe-digit l)) ∙ ∥ x ∥ ∙ (measure-maybe-digit r ∙ μ₂ ∙ σsf)
      ∎
      where
        σpr = measure-digit pr
        σsf = measure-digit sf
        μ₁ = measure-tree ml
        μ₂ = measure-tree mr
...
foldr-dig : ∀ {a} {A : Set a} {V : Set a}
      → (A → V → V) → Digit A → V → V
foldr-dig f (One x) i = f x i
foldr-dig f (Two x x₁) i = f x (f x₁ i)
foldr-dig f (Three x x₁ x₂) i = f x (f x₁ (f x₂ i))
foldr-dig f (Four x x₁ x₂ x₃) i = f x (f x₁ (f x₂ (f x₃ i)))
...
assoc-lemma6 {V = V} ⦃ mo ⦄ = assoc-lemma6'
   where
      open MonoidSolver V mo
      assoc-lemma6' = assoc-lemma-5ms

assoc-lemma7 : ∀ {l} {V : Set l} ⦃ mo : Monoid V ⦄
            → (a : V) → (b : V)
            → (ε ∙ a) ∙ b ≡ (a ∙ ε) ∙ b
assoc-lemma7 {V = V} ⦃ mo ⦄ = assoc-lemma7'
    where
      open MonoidSolver V mo
      assoc-lemma7' = assoc-lemma-7ms
...
measure-list : ∀ {a} {A : Set a} {V : Set a}
          ...
          → V
measure-list [] = ε
measure-list (x ∷ xs) = ∥ x ∥ ∙ (measure-list xs)

foldfun-lemma : ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (x : V)
          → (y : A)
          → (x ∙ (foldfun ε y) ≡ foldfun x y)
foldfun-lemma x y =
  begin
    x ∙ (foldfun ε y)
  ≡⟨ refl ⟩
...
fold-monoid-lemma : ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (x : V)
          → (xs : List A)
          → (x ∙ Data.List.foldl foldfun ε xs ≡ Data.List.foldl foldfun x xs)
fold-monoid-lemma x [] = ε-right x
fold-monoid-lemma x (y ∷ xs) =
  begin
    x ∙ (Data.List.foldl foldfun (foldfun ε y) xs)
  ≡⟨ cong (x ∙_) (sym (fold-monoid-lemma (foldfun ε y) xs)) ⟩
    x ∙ (foldfun ε y) ∙ (Data.List.foldl foldfun ε xs)
  ≡⟨ ∙-assoc x (foldfun ε y) (Data.List.foldl foldfun ε xs) ⟩
    (x ∙ (foldfun ε y)) ∙ (Data.List.foldl foldfun ε xs)
  ≡⟨ cong (_∙ (Data.List.foldl foldfun ε xs)) (foldfun-lemma x y) ⟩
    (foldfun x y) ∙ (Data.List.foldl foldfun ε xs)
  ≡⟨ fold-monoid-lemma (foldfun x y) xs ⟩
    Data.List.foldl foldfun (foldfun x y) xs
  ∎

measure-list-lemma : ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (xs : List A)
          → (measure-list xs ≡ Data.List.foldl foldfun (Monoid.ε mo) xs)
measure-list-lemma [] = refl
measure-list-lemma (x ∷ xs) =
  begin
    ∥ x ∥ ∙ (measure-list xs)
  ≡⟨ cong (∥ x ∥ ∙_) (measure-list-lemma xs) ⟩
    ∥ x ∥ ∙ (Data.List.foldl foldfun ε xs)
  ≡⟨ fold-monoid-lemma (∥ x ∥) xs ⟩
    Data.List.foldl foldfun ∥ x ∥ xs
  ≡⟨ cong (λ a → Data.List.foldl foldfun a xs) (sym (ε-left ∥ x ∥)) ⟩
    Data.List.foldl foldfun (ε ∙ ∥ x ∥) xs
  ≡⟨ cong (λ a → Data.List.foldl foldfun a xs) refl ⟩
    Data.List.foldl foldfun (foldfun ε x) xs
  ∎

append-measure-lemma :  ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (μ : V)
          → (x : A)
          ...
          → (μ ∙ (measure-list (x ∷ xs)) ≡
             (μ ∙ ∥ x ∥) ∙ (measure-list xs))
append-measure-lemma μ x xs =
  begin
    μ ∙ (measure-list (x ∷ xs))
  ≡⟨ cong (μ ∙_) refl ⟩
    (μ ∙ ((∥ x ∥) ∙ (measure-list xs)))
  ≡⟨ ∙-assoc μ (∥ x ∥) (measure-list xs) ⟩
    (μ ∙ ∥ x ∥) ∙ (measure-list xs)
  ∎

toList-ft-measure-correct : ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          {μ : V}
          → (xs : FingerTree A V {μ})
          → (measure-list (toList-ft xs) ≡ measure-tree xs)
toList-ft-measure-correct xs =
  begin
    measure-list (toList-ft xs)
  ≡⟨ measure-list-lemma (toList-ft xs) ⟩
    Data.List.foldl foldfun ε (toList-ft xs)
  ≡⟨ sym (foldl-correct foldfun ε xs) ⟩
    foldl foldfun ε xs
  ≡⟨ foldl-lemma0 ε xs ⟩
    ε ∙ (measure-tree xs)
  ≡⟨ ε-left (measure-tree xs) ⟩
    measure-tree xs
  ∎
...
append : ∀ {a} {A : Set a} {V : Set a}
          ...
          → FingerTree A V {μ}
          → (ys : List A)
          → FingerTree A V {μ ∙ (measure-list ys)}
append xs [] rewrite ε-right (measure-tree xs)
      = xs
append xs (y ∷ ys) rewrite (append-measure-lemma (measure-tree xs) y ys)
      = append (y ▷ xs) ys

concat :  ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          {μ : V} {σ : V}
          → FingerTree A V {μ}
          → FingerTree A V {σ}
          → FingerTree A V {μ ∙ σ}
concat xs ys rewrite sym (toList-ft-measure-correct ys)
      = append xs (toList-ft ys)
...
rev-digit : ∀ {a} {A : Set a}
          ...
          → Digit A
          → Digit A
rev-digit rv (One x) = One (rv x)
rev-digit rv (Two x x₁) = Two (rv x₁) (rv x)
rev-digit rv (Three x x₁ x₂) = Three (rv x₂) (rv x₁) (rv x)
rev-digit rv (Four x x₁ x₂ x₃) = Four (rv x₃) (rv x₂) (rv x₁) (rv x)

rev-node : ∀ {a} {A : Set a} {V : Set a}
           ⦃ m : Monoid V ⦄
           ⦃ m : Measured A V ⦄
          → (rv : A → A)
          → Node A V
          → Node A V
rev-node rv (Node2 v x y x₁) = node2 (rv y) (rv x)
rev-node rv (Node3 v x y z x₁) = node3 (rv z) (rv y) (rv x)

measure-rev-list : ∀ {a} {A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (rv : A → A)
          → (xs : List A)
          → V
measure-rev-list rv xs = Data.List.foldl foldfun ε (Data.List.reverse (Data.List.map rv xs))

measure-rev : ∀ {a}{A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          {μ : V}
          → (rv : A → A)
          → (xs : FingerTree A V {μ})
          → V
measure-rev rv xs = Data.List.foldl foldfun ε (Data.List.reverse (Data.List.map rv (toList-ft xs)))

measure-digit-rev : ∀ {a}{A : Set a} {V : Set a}
          ⦃ mo : Monoid V ⦄
          ⦃ m : Measured A V ⦄
          → (rv : A → A)
          → (d : Digit A)
