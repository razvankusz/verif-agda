...
open import Size
open import Agda.Builtin.Size using (_⊔ˢ_)
...
data Digit {a : Level} (A : Set a) : {i : Size} → Set a where
  One   : ∀ {i} → A → Digit A {↑ i}
  Two   : ∀ {i} → A → A → Digit A {↑ i}
  Three : ∀ {i} → A → A → A → Digit A {↑ i}
  Four  : ∀ {i} → A → A → A → A → Digit A {↑ i}

data FingerTree {a : Level} (A : Set a)(V : Set a ) : {i : Size} → Set a where
  Empty  : ∀ {i : Size} → FingerTree A V {↑ i}
  Single : ∀ {i : Size} → A → FingerTree A V {↑ i}
  Deep   : ∀ {i j k} → V → Digit A {j} → FingerTree (Node A V) V {i} → Digit A {k} →
           FingerTree A V {↑ ((j ⊔ˢ i) ⊔ˢ k)}
...
measure-tree : {a : Level}{i : Size}{A : Set a}{V : Set a } ⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄ → FingerTree A V {i} → V
-- measure-tree Empty = ε
-- measure-tree (Single x) = ∥ x ∥
-- measure-tree (Deep v x ft x₁) = v
measure-tree Empty = ε
measure-tree (Single x) = ∥ x ∥
measure-tree (Deep x x₁ ft x₂) = x

instance uplift : ∀ {a}{A : Set a}{V : Set a } ⦃ m : Measured A V ⦄ → (Measured (Node A V) V)
uplift  = measured measure-node

deep : ∀ {a : Level} {i j k} {A : Set a}{V : Set a } ⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄ →
  (pr : Digit A {j}) → (ft : FingerTree (Node A V) V {i}) → (sf : Digit A {k}) →
  FingerTree A V {↑ ((j ⊔ˢ i) ⊔ˢ k)}
deep {_}{i}{j}{k} pr ft sf = Deep {i = i}{j = j}{k = k} {!   !} pr ft sf --  -- ?Deep {!   !} pr ft sf -- Deep ((measure-digit pr) ∙ ((measure-tree ft) ∙ (measure-digit sf))) pr ft sf

size-d : ∀ {a}{i}{A : Set a} → Digit A {i} → Size
size-d {i = i} d = i

size-t : ∀ {a}{i}{A : Set a}{V : Set a } ⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄ → FingerTree A V {i} → Size
size-t {i = i} ft = i
...
_◁_ a (Deep v (One {j} b) ft sf) = deep   {!   !} {!   !} {!   !} -- deep (Two a b) ft sf
...
toTree-dig : ∀ {a}{i}{A : Set a}{V : Set a } ⦃ mo : Monoid V ⦄ ⦃ m : Measured A V ⦄ →
      Digit A → FingerTree A V {i}
toTree-dig d = {!   !}
--
-- -- To List ----------------------------------------------------------
...
toList-ft : ∀ {a}{i}{A : Set a}{V : Set a } → FingerTree A V {i} → List A
toList-ft Empty = []
...
mutual
  ...
  deepL nothing ft sf | NilL = {!   !} -- toTree-dig sf
