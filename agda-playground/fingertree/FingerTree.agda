...
_◁′_ : ∀ {a i} {F : Set a → Set a} {{r : reduceClass F}} {A : Set a} →
       F A → FingerTree A {i} → FingerTree A {↑ i}
_◁′_ = reducer _◁_
...
toTree : ∀ {a i} {F : Set a → Set a} {{r : reduceClass F}} {A : Set a} →
         F A → FingerTree A {↑ i}
toTree {i = i} s = s ◁′ Empty {i = i}
...
--
-- mutual
--   viewL : ∀ {i a} {A : Set a} → FingerTree A {i} → ViewL {i} {a} A
--   viewL Empty = nilL
--   viewL (Single x) = consL x Empty
--   viewL (Deep pr m sf) = consL (head pr) (deepL (tail pr) m sf)
--
--   deepL : ∀ {a i} {A : Set a} → Maybe (Digit A) →
--            FingerTree (Node A) {i} → Digit A → FingerTree A {↑ i}
