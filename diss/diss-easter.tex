...
Furthermore, in this form, we no longer work with dependently typed functions, since the indexes have been covered up by the dependent pair. No correctness is enforced by the typing system. Trying to prove correctness of this function brings back the problems caused by \textit{with}. 
In particular, we have an induction proving mechanism, but we are stopped by the inability to prove an inductive step on viewL.
...
\\
\>[B]{}\Varid{property}\;\Varid{xs}\mathrel{=}\Varid{to}\mathbin{-}\Varid{size}\;(\Varid{rev}\;\Varid{xs})\mathbin{≡}\Varid{to}\mathbin{-}\Varid{size}\;\Varid{xs}{}\<[E]%
\\
\>[B]{}\mbox{\onelinecomment  example property}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
The problem that remains is proving the inductive step: 
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{11}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
- implement reverse without viewl? 
